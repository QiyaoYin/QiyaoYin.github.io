...
let obj = {
  ...
  next(){
    if(this.index < this.value.length){
      return {done: false,value: this.value[this.index++]}
    }
    return {done: true,value: this.value[this.index]}
  }
}
...
let my = new myArray(1,2,3,4,5,6);
for(let m of my){
  console.log(m); //1 2 3 4 5 6
}
```

> class定义[Symbol.iterator]
...
class myArray{
  ...
  next(){
    let flag = this.index >= this.length;
    return {done: flag,value: this.value[flag ? this.index : this.index++]}
  }
}

let my = new myArray(1,2,3,4);
for(let m of my){
...
阮一峰-[Iterator 和 for...of 循环]([https://es6.ruanyifeng.com/#docs/iterator#%E9%81%8D%E5%8E%86%E5%99%A8%E5%AF%B9%E8%B1%A1%E7%9A%84-return%EF%BC%8Cthrow](https://es6.ruanyifeng.com/#docs/iterator#遍历器对象的-return，throw))